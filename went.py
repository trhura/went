...
# Time-stamp: <2017-11-27 09:12:50 (thurahlaing)>
...
class RecentlyVisitedDirectories:
    ...
    def get_recently_visited_directories (self):
        return self.pathsbydir.keys()

    def get_recently_visited (self, dirname):
        if dirname not in self.pathsbydir: return ""

        path = self.pathsbydir[dirname][0]
        if os.path.exists(path):
            return path

        del self.pathsbydir[dirname][0]  # delete if the path no longer exists
        # delete directory if no paths exist
        if len(self.pathsbydir[dirname]) == 0: del self.pathsbydir[dirname] # noqa
...
def main():
    ...
    if destination == "__completions__":
        print(" ".join(RECENT_DIRECTORIES.get_recently_visited_directories()))
    else:
